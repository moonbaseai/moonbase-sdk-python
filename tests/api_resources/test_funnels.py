# File generated from our OpenAPI spec by Stainless. See CONTRIBUTING.md for details.

from __future__ import annotations

import os
from typing import Any, cast

import pytest

from moonbase import Moonbase, AsyncMoonbase
from tests.utils import assert_matches_type
from moonbase.types import Funnel
from moonbase.pagination import SyncCursorPage, AsyncCursorPage

base_url = os.environ.get("TEST_API_BASE_URL", "http://127.0.0.1:4010")


class TestFunnels:
    parametrize = pytest.mark.parametrize("client", [False, True], indirect=True, ids=["loose", "strict"])

    @parametrize
    def test_method_create(self, client: Moonbase) -> None:
        funnel = client.funnels.create(
            name="Sales Pipeline",
        )
        assert_matches_type(Funnel, funnel, path=["response"])

    @parametrize
    def test_method_create_with_all_params(self, client: Moonbase) -> None:
        funnel = client.funnels.create(
            name="Sales Pipeline",
            steps=[
                {
                    "color": "blue",
                    "name": "New Lead",
                    "step_type": "active",
                },
                {
                    "color": "cyan",
                    "name": "Qualified",
                    "step_type": "active",
                },
                {
                    "color": "green",
                    "name": "Won",
                    "step_type": "success",
                },
                {
                    "color": "red",
                    "name": "Lost",
                    "step_type": "failure",
                },
            ],
        )
        assert_matches_type(Funnel, funnel, path=["response"])

    @parametrize
    def test_raw_response_create(self, client: Moonbase) -> None:
        response = client.funnels.with_raw_response.create(
            name="Sales Pipeline",
        )

        assert response.is_closed is True
        assert response.http_request.headers.get("X-Stainless-Lang") == "python"
        funnel = response.parse()
        assert_matches_type(Funnel, funnel, path=["response"])

    @parametrize
    def test_streaming_response_create(self, client: Moonbase) -> None:
        with client.funnels.with_streaming_response.create(
            name="Sales Pipeline",
        ) as response:
            assert not response.is_closed
            assert response.http_request.headers.get("X-Stainless-Lang") == "python"

            funnel = response.parse()
            assert_matches_type(Funnel, funnel, path=["response"])

        assert cast(Any, response.is_closed) is True

    @parametrize
    def test_method_retrieve(self, client: Moonbase) -> None:
        funnel = client.funnels.retrieve(
            "id",
        )
        assert_matches_type(Funnel, funnel, path=["response"])

    @parametrize
    def test_raw_response_retrieve(self, client: Moonbase) -> None:
        response = client.funnels.with_raw_response.retrieve(
            "id",
        )

        assert response.is_closed is True
        assert response.http_request.headers.get("X-Stainless-Lang") == "python"
        funnel = response.parse()
        assert_matches_type(Funnel, funnel, path=["response"])

    @parametrize
    def test_streaming_response_retrieve(self, client: Moonbase) -> None:
        with client.funnels.with_streaming_response.retrieve(
            "id",
        ) as response:
            assert not response.is_closed
            assert response.http_request.headers.get("X-Stainless-Lang") == "python"

            funnel = response.parse()
            assert_matches_type(Funnel, funnel, path=["response"])

        assert cast(Any, response.is_closed) is True

    @parametrize
    def test_path_params_retrieve(self, client: Moonbase) -> None:
        with pytest.raises(ValueError, match=r"Expected a non-empty value for `id` but received ''"):
            client.funnels.with_raw_response.retrieve(
                "",
            )

    @parametrize
    def test_method_update(self, client: Moonbase) -> None:
        funnel = client.funnels.update(
            id="id",
        )
        assert_matches_type(Funnel, funnel, path=["response"])

    @parametrize
    def test_method_update_with_all_params(self, client: Moonbase) -> None:
        funnel = client.funnels.update(
            id="id",
            name="Revenue Pipeline",
            steps=[
                {
                    "color": "amber",
                    "name": "name",
                    "step_type": "active",
                    "id": "id",
                }
            ],
        )
        assert_matches_type(Funnel, funnel, path=["response"])

    @parametrize
    def test_raw_response_update(self, client: Moonbase) -> None:
        response = client.funnels.with_raw_response.update(
            id="id",
        )

        assert response.is_closed is True
        assert response.http_request.headers.get("X-Stainless-Lang") == "python"
        funnel = response.parse()
        assert_matches_type(Funnel, funnel, path=["response"])

    @parametrize
    def test_streaming_response_update(self, client: Moonbase) -> None:
        with client.funnels.with_streaming_response.update(
            id="id",
        ) as response:
            assert not response.is_closed
            assert response.http_request.headers.get("X-Stainless-Lang") == "python"

            funnel = response.parse()
            assert_matches_type(Funnel, funnel, path=["response"])

        assert cast(Any, response.is_closed) is True

    @parametrize
    def test_path_params_update(self, client: Moonbase) -> None:
        with pytest.raises(ValueError, match=r"Expected a non-empty value for `id` but received ''"):
            client.funnels.with_raw_response.update(
                id="",
            )

    @parametrize
    def test_method_list(self, client: Moonbase) -> None:
        funnel = client.funnels.list()
        assert_matches_type(SyncCursorPage[Funnel], funnel, path=["response"])

    @parametrize
    def test_method_list_with_all_params(self, client: Moonbase) -> None:
        funnel = client.funnels.list(
            after="after",
            before="before",
            limit=1,
        )
        assert_matches_type(SyncCursorPage[Funnel], funnel, path=["response"])

    @parametrize
    def test_raw_response_list(self, client: Moonbase) -> None:
        response = client.funnels.with_raw_response.list()

        assert response.is_closed is True
        assert response.http_request.headers.get("X-Stainless-Lang") == "python"
        funnel = response.parse()
        assert_matches_type(SyncCursorPage[Funnel], funnel, path=["response"])

    @parametrize
    def test_streaming_response_list(self, client: Moonbase) -> None:
        with client.funnels.with_streaming_response.list() as response:
            assert not response.is_closed
            assert response.http_request.headers.get("X-Stainless-Lang") == "python"

            funnel = response.parse()
            assert_matches_type(SyncCursorPage[Funnel], funnel, path=["response"])

        assert cast(Any, response.is_closed) is True

    @parametrize
    def test_method_delete(self, client: Moonbase) -> None:
        funnel = client.funnels.delete(
            "id",
        )
        assert funnel is None

    @parametrize
    def test_raw_response_delete(self, client: Moonbase) -> None:
        response = client.funnels.with_raw_response.delete(
            "id",
        )

        assert response.is_closed is True
        assert response.http_request.headers.get("X-Stainless-Lang") == "python"
        funnel = response.parse()
        assert funnel is None

    @parametrize
    def test_streaming_response_delete(self, client: Moonbase) -> None:
        with client.funnels.with_streaming_response.delete(
            "id",
        ) as response:
            assert not response.is_closed
            assert response.http_request.headers.get("X-Stainless-Lang") == "python"

            funnel = response.parse()
            assert funnel is None

        assert cast(Any, response.is_closed) is True

    @parametrize
    def test_path_params_delete(self, client: Moonbase) -> None:
        with pytest.raises(ValueError, match=r"Expected a non-empty value for `id` but received ''"):
            client.funnels.with_raw_response.delete(
                "",
            )


class TestAsyncFunnels:
    parametrize = pytest.mark.parametrize(
        "async_client", [False, True, {"http_client": "aiohttp"}], indirect=True, ids=["loose", "strict", "aiohttp"]
    )

    @parametrize
    async def test_method_create(self, async_client: AsyncMoonbase) -> None:
        funnel = await async_client.funnels.create(
            name="Sales Pipeline",
        )
        assert_matches_type(Funnel, funnel, path=["response"])

    @parametrize
    async def test_method_create_with_all_params(self, async_client: AsyncMoonbase) -> None:
        funnel = await async_client.funnels.create(
            name="Sales Pipeline",
            steps=[
                {
                    "color": "blue",
                    "name": "New Lead",
                    "step_type": "active",
                },
                {
                    "color": "cyan",
                    "name": "Qualified",
                    "step_type": "active",
                },
                {
                    "color": "green",
                    "name": "Won",
                    "step_type": "success",
                },
                {
                    "color": "red",
                    "name": "Lost",
                    "step_type": "failure",
                },
            ],
        )
        assert_matches_type(Funnel, funnel, path=["response"])

    @parametrize
    async def test_raw_response_create(self, async_client: AsyncMoonbase) -> None:
        response = await async_client.funnels.with_raw_response.create(
            name="Sales Pipeline",
        )

        assert response.is_closed is True
        assert response.http_request.headers.get("X-Stainless-Lang") == "python"
        funnel = await response.parse()
        assert_matches_type(Funnel, funnel, path=["response"])

    @parametrize
    async def test_streaming_response_create(self, async_client: AsyncMoonbase) -> None:
        async with async_client.funnels.with_streaming_response.create(
            name="Sales Pipeline",
        ) as response:
            assert not response.is_closed
            assert response.http_request.headers.get("X-Stainless-Lang") == "python"

            funnel = await response.parse()
            assert_matches_type(Funnel, funnel, path=["response"])

        assert cast(Any, response.is_closed) is True

    @parametrize
    async def test_method_retrieve(self, async_client: AsyncMoonbase) -> None:
        funnel = await async_client.funnels.retrieve(
            "id",
        )
        assert_matches_type(Funnel, funnel, path=["response"])

    @parametrize
    async def test_raw_response_retrieve(self, async_client: AsyncMoonbase) -> None:
        response = await async_client.funnels.with_raw_response.retrieve(
            "id",
        )

        assert response.is_closed is True
        assert response.http_request.headers.get("X-Stainless-Lang") == "python"
        funnel = await response.parse()
        assert_matches_type(Funnel, funnel, path=["response"])

    @parametrize
    async def test_streaming_response_retrieve(self, async_client: AsyncMoonbase) -> None:
        async with async_client.funnels.with_streaming_response.retrieve(
            "id",
        ) as response:
            assert not response.is_closed
            assert response.http_request.headers.get("X-Stainless-Lang") == "python"

            funnel = await response.parse()
            assert_matches_type(Funnel, funnel, path=["response"])

        assert cast(Any, response.is_closed) is True

    @parametrize
    async def test_path_params_retrieve(self, async_client: AsyncMoonbase) -> None:
        with pytest.raises(ValueError, match=r"Expected a non-empty value for `id` but received ''"):
            await async_client.funnels.with_raw_response.retrieve(
                "",
            )

    @parametrize
    async def test_method_update(self, async_client: AsyncMoonbase) -> None:
        funnel = await async_client.funnels.update(
            id="id",
        )
        assert_matches_type(Funnel, funnel, path=["response"])

    @parametrize
    async def test_method_update_with_all_params(self, async_client: AsyncMoonbase) -> None:
        funnel = await async_client.funnels.update(
            id="id",
            name="Revenue Pipeline",
            steps=[
                {
                    "color": "amber",
                    "name": "name",
                    "step_type": "active",
                    "id": "id",
                }
            ],
        )
        assert_matches_type(Funnel, funnel, path=["response"])

    @parametrize
    async def test_raw_response_update(self, async_client: AsyncMoonbase) -> None:
        response = await async_client.funnels.with_raw_response.update(
            id="id",
        )

        assert response.is_closed is True
        assert response.http_request.headers.get("X-Stainless-Lang") == "python"
        funnel = await response.parse()
        assert_matches_type(Funnel, funnel, path=["response"])

    @parametrize
    async def test_streaming_response_update(self, async_client: AsyncMoonbase) -> None:
        async with async_client.funnels.with_streaming_response.update(
            id="id",
        ) as response:
            assert not response.is_closed
            assert response.http_request.headers.get("X-Stainless-Lang") == "python"

            funnel = await response.parse()
            assert_matches_type(Funnel, funnel, path=["response"])

        assert cast(Any, response.is_closed) is True

    @parametrize
    async def test_path_params_update(self, async_client: AsyncMoonbase) -> None:
        with pytest.raises(ValueError, match=r"Expected a non-empty value for `id` but received ''"):
            await async_client.funnels.with_raw_response.update(
                id="",
            )

    @parametrize
    async def test_method_list(self, async_client: AsyncMoonbase) -> None:
        funnel = await async_client.funnels.list()
        assert_matches_type(AsyncCursorPage[Funnel], funnel, path=["response"])

    @parametrize
    async def test_method_list_with_all_params(self, async_client: AsyncMoonbase) -> None:
        funnel = await async_client.funnels.list(
            after="after",
            before="before",
            limit=1,
        )
        assert_matches_type(AsyncCursorPage[Funnel], funnel, path=["response"])

    @parametrize
    async def test_raw_response_list(self, async_client: AsyncMoonbase) -> None:
        response = await async_client.funnels.with_raw_response.list()

        assert response.is_closed is True
        assert response.http_request.headers.get("X-Stainless-Lang") == "python"
        funnel = await response.parse()
        assert_matches_type(AsyncCursorPage[Funnel], funnel, path=["response"])

    @parametrize
    async def test_streaming_response_list(self, async_client: AsyncMoonbase) -> None:
        async with async_client.funnels.with_streaming_response.list() as response:
            assert not response.is_closed
            assert response.http_request.headers.get("X-Stainless-Lang") == "python"

            funnel = await response.parse()
            assert_matches_type(AsyncCursorPage[Funnel], funnel, path=["response"])

        assert cast(Any, response.is_closed) is True

    @parametrize
    async def test_method_delete(self, async_client: AsyncMoonbase) -> None:
        funnel = await async_client.funnels.delete(
            "id",
        )
        assert funnel is None

    @parametrize
    async def test_raw_response_delete(self, async_client: AsyncMoonbase) -> None:
        response = await async_client.funnels.with_raw_response.delete(
            "id",
        )

        assert response.is_closed is True
        assert response.http_request.headers.get("X-Stainless-Lang") == "python"
        funnel = await response.parse()
        assert funnel is None

    @parametrize
    async def test_streaming_response_delete(self, async_client: AsyncMoonbase) -> None:
        async with async_client.funnels.with_streaming_response.delete(
            "id",
        ) as response:
            assert not response.is_closed
            assert response.http_request.headers.get("X-Stainless-Lang") == "python"

            funnel = await response.parse()
            assert funnel is None

        assert cast(Any, response.is_closed) is True

    @parametrize
    async def test_path_params_delete(self, async_client: AsyncMoonbase) -> None:
        with pytest.raises(ValueError, match=r"Expected a non-empty value for `id` but received ''"):
            await async_client.funnels.with_raw_response.delete(
                "",
            )
